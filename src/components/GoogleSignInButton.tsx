...
export default function GoogleSignInButton({ label = "Sign in with Google" }: { label?: string }) {
  const { signIn, status } = useGoogleAuth();
  ...
  return (
    <button
      ...
      onClick={signIn}
      className="inline-flex h-12 items-center justify-center gap-3 rounded-full border border-black/[.12] bg-white px-6 text-base font-medium text-zinc-900 shadow-sm transition-colors hover:bg-zinc-50 disabled:opacity-60 dark:border-white/[.16] dark:bg-zinc-900 dark:text-zinc-50 dark:hover:bg-zinc-800"
    >
      <GoogleLogo />
